...
import { vignette } from './vignettes.js?v=mue7a3c0';
import { BRAND, marque, blocMarque } from './brand.js?v=mue7a3c0';
import { LANG, T, champ, poserSelecteur } from './langue.js?v=mue7a3c0';
import { miroirHTML, animerMiroir } from './miroir.js?v=mue7a3c0';
...
/* Le compte de Fable. Il est chargé par un script classique posé avant ce
   module ; absent — ouvert hors du site publié — on n’affiche rien. */
if (window.FableCompte) window.FableCompte.poserBouton(document.getElementById('compte-hote'));
...
async function load() {
  const r = await fetch('catalogue.json?v=mue7a3c0');
  DATA = await r.json();
  cycleDuFoyer();
  render();
}
